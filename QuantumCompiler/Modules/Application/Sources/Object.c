...
{
  const DataTypeInfo* Info = DataType_Find(pDataType);
  if (Info == NULL)
    // TODO Exception 처리
    return NULL;
  return Info->m_Option == DataTypeIsSystem ? Info->m_SystemInfo->m_Boxing
                                            : Info->m_CustemInfo->m_Boxing;
}
...
__ObjectUnBoxingSearch(const char* pDataType)
{
  ...
  if (Info == NULL)
    // TODO Exception 처리
    return NULL;
  return Info->m_Option == DataTypeIsSystem ? Info->m_SystemInfo->m_UnBoxing
                                            : Info->m_CustemInfo->m_UnBoxing;
}
