...
{
  Events event = &Application.ProcessEvent[ProcessEvent_Quit];

  FuncChainNode* ptr = (FuncChainNode*)malloc(sizeof(FuncChainNode));
  if (ptr == NULL) {
    Exception(ERROR,
              "Quit 함수 노드를 생성하지 못했습니다. [size:%lu]",
...
RemoveListener(Func_t Method)
{
  ...
  Exception(ERROR, "할당한 Quit 함수가 아닙니다. [func:%p]", Method);
}
...
Invoke()
{
  FuncChainNode* Pos = Application.ProcessEvent[ProcessEvent_Quit].m_Nodes;
  while (Pos != NULL) {
    Pos->Callback();
    Pos = Pos->Next;
  }
