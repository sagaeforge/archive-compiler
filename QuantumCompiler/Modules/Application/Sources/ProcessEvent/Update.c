...
UpdateStart()
{
  if (Application.ProcessEvent[ProcessEvent_Update].m_Nodes == NULL)
    return;
...
Invoke()
{
  FuncChainNode* Pos = Application.ProcessEvent[ProcessEvent_Update].m_Nodes;
  while (Pos != NULL) {
    Pos->Callback();
    Pos = Pos->Next;
  }
...
ProcessEventModule_Update_Initialized()
{
  ...
  Application.Member.ProcessEvent_UpdateStart = UpdateStart;
  Application.Member.ProcessEvent_UpdateStop = UpdateStop;
  Application.Member.ProcessEvent_UpdateWaitStop = UpdateStopWait;
}