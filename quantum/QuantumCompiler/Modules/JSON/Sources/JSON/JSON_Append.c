...
bool
JSON_Append(JSONObject pSelf, const String pFieldName, const Object pValue)
{
  JSONNode MakeNode = JSON_NodeCreate();
  MakeNode->m_Name = pFieldName;
  ...
  if (DTC == DataType_Int) {
    MakeNode->m_DataType = JSONDataType_Decimal;
    if (value_int != 0) {
      MakeNode->m_Value.StringValue = toString(value_int);
    } else {
      MakeNode->m_Value.StringValue = toString(value_uint);
    }
  } else if (DTC == DataType_Float) {
    MakeNode->m_DataType = JSONDataType_Digit;
    MakeNode->m_Value.StringValue = toString(value_double);
  } else if (DTC == DataType_Bool) {
    MakeNode->m_DataType = JSONDataType_Boolean;
    MakeNode->m_Value.StringValue = toString(value_boolean);
  } else if (DTC == DataType_String) {
    MakeNode->m_DataType = JSONDataType_String;
    MakeNode->m_Value.StringValue = value_string;
  } else if (DTC == DataType_JSONObject) {
    MakeNode->m_DataType = JSONDataType_JSONObject;
    MakeNode->m_Value.ReferenceValue = value_jsonobject;
    value_jsonobject->m_Parent = pSelf;
  } else if (DTC == DataType_JSONAry) {
    MakeNode->m_DataType = JSONDataType_Ary;
    MakeNode->m_Value.ReferenceValue = value_jsonary;
  } else {
    if (value_void != NULL) {
      Exception(ERROR,
    ...
    MakeNode->m_Value.ReferenceValue = NULL;
  }
  ...
  StringAryMethod.Push(pSelf->m_FieldNames, pFieldName);
  pSelf->m_FieldLength++;
