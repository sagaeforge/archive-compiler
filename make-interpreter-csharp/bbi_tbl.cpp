...
/* ---------------------------- 테이블 */
vector<Symbol> Gtable;            /* 전역 심볼 테이블 */
vector<Symbol> Ltable;            /* 로컬 심볼 테이블 */
int startLtable;           		  /* 로컬용 시작 위치 */

int enter(Symbol& tb, SymKind kind) /* 심볼 테이블 등록 */
{
	int n, mem_size;
...
vector<Symbol>::iterator tableP(const CodeSet& cd) /* 반복자 획득 */
{
