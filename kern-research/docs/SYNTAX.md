...
// 단형화(monomorphization)로 구현
// 컴파일 타임에 사용된 타입별로 함수 인스턴스 생성
```
...
| # | 질문 | 상태 |
|---|------|------|
| ~~SQ-1~~ | ~~bind의 구체적 형태~~ | **결정됨: 파이프 + 패턴매칭으로 처리** |
| ~~SQ-2~~ | ~~구조체/레코드 타입 문법~~ | **구현됨: `struct Name { field: Type }` (M5a)** |
| ~~SQ-3~~ | ~~열거형(enum/ADT) 문법~~ | **구현됨: `enum`/`union` (M5b)** |
| ~~SQ-4~~ | ~~모듈/임포트 문법~~ | **구현됨: `module`/`import` (Phase B)** |
| ~~SQ-5~~ | ~~연산자 오버로딩~~ | **결정됨: 미지원 (명시적 함수 호출 선호)** |
| ~~SQ-6~~ | ~~문자열 리터럴과 보간~~ | **구현됨: String = fat pointer, 이스케이프 시퀀스 (M5d)** |
| ~~SQ-7~~ | ~~블록 주석~~ | **결정됨: `//` + `/* */` 둘 다 지원** |
| ~~SQ-8~~ | ~~제네릭 문법~~ | **구현됨: `<>` + 단형화, 터보피시 없음** |
| ~~SQ-9~~ | ~~역참조 연산자~~ | **구현됨: `*ptr` (C/Rust 스타일)** |
| ~~SQ-10~~ | ~~논리 연산자~~ | **결정됨: `and`/`or`/`not` 키워드** |
| ~~SQ-11~~ | ~~포인터 멤버 접근~~ | **구현됨: 명시적 역참조 필수 `(*ptr).field`** |
...
모든 문법 요소가 구현 완료됨. v2 파이프라인 (4-level IR) 사용:
```
