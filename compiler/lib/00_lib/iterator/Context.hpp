...
template <typename T> class Context {
  ...
  public:
    iterator_t &current() { return m_current; }

  public:
    Context(const Context &other) : m_values{other.m_values}, m_current{*this, other.m_current.position()} {}
    Context(Context &&other) noexcept : m_values{std::move(other.m_values)}, m_current{std::move(other.m_current)} {}
    Context(const std::vector<T> &values) : m_values{values}, m_current{*this, 0} {}

  public:
    elem_t operator[](const iterator_t::position_t &position) const { return m_values[position]; }
...
template <typename T> using context_t = Context<T>;
template <typename T> class ContextCommand {
  public:
  ...
  public:
    ...
    iterator_t::position_t position() const { return m_context.current().position(); }
    iterator_t::elem_t value() const { return m_context.current().value(); }
    iterator_t current() const { return m_context.current(); }
    bool valid() const { return m_context.valid(); }

  private:
    Context<T> &m_context;
};
template <typename T> using command_t = ContextCommand<T>;
