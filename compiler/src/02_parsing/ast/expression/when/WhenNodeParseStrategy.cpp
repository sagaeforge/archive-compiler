#include "WhenNodeParseStrategy.h"
...
bool WhenNodeParseStrategy::can_parse(const tokenize::TokenStream &tokens) { return contains(tokens.current(), {tokenize::TokenType::When}); }

parsing::ParseStrategyResult WhenNodeParseStrategy::parse(const tokenize::TokenStream &tokens) {
    ...
    auto workbench = tokens.clone(); // current : when
    workbench.next();
    ...
    if (!contains(workbench.current(), {tokenize::TokenType::LBrace})) {
        throw std::runtime_error("Expected '{' after when");
    }

    // 여기는 expression 여러개 존재할수 있음.
    bool isAlternative = false;
    WhenNode::Consequence alternative = {};
    std::map<std::shared_ptr<Expression>, WhenNode::Consequence> conditions;
    do {
        workbench.next();

        // 선두에 올 수 있는 식 먼저 확인하고, '->' 다음 statement 혹은
        std::shared_ptr<Expression> condition = nullptr;
        if (contains(workbench.current(), {tokenize::TokenType::In})) {
            auto token = *workbench.current();
            if (target == nullptr) {
                throw std::runtime_error("Expected target before 'in' in when");
            }

            auto [expressionNode, expressionItr] = infixExpressionNodeParseStrategy.parse(workbench, target);
        ...
        } else if (contains(workbench.current(), {tokenize::TokenType::Else})) {
            if (isAlternative) {
        ...
        if (!contains(workbench.current(), {tokenize::TokenType::LeftArrow})) {
    ...
    return {std::make_shared<WhenNode>(*tokens.current(), target, conditions, alternative), tokens.begin() + workbench.current().distance()};
}
