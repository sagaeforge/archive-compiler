...
WhenNode::WhenNode(const tokenize::Token &token, std::shared_ptr<Expression> target, std::map<std::shared_ptr<Expression>, Consequence> conditions,
                   Consequence alternative)
    : m_token(token), m_target(target), m_conditions(conditions), m_alternative(alternative) {}

icu::UnicodeString WhenNode::to_str() const { return u"when"; }

json::JsonValue WhenNode::to_json(json::JsonAllocator &allocator) const {
...
const tokenize::Token &WhenNode::get_token() const { return m_token; }
