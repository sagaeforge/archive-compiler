...
bool ContinueStatementNodeParseStrategy::can_parse(const tokenize::TokenStream &tokens) { return contains(tokens.current(), {tokenize::TokenType::Continue}); }

parsing::ParseStrategyResult ContinueStatementNodeParseStrategy::parse(const tokenize::TokenStream &tokens) {
    ...
    auto [node, itr] = stream::workbench(tokens, [this, &tokens](tokenize::TokenStream &workbench) {
        ...
        auto continueNode = std::make_shared<ContinueStatementNode>(tokens.current().value(), static_cast<std::shared_ptr<Expression>>(nullptr));
