...
bool ReturnStatementNodeParseStrategy::can_parse(const tokenize::TokenStream &tokens) {
    return tokens.current().value().get_type() == tokenize::TokenType::Return;
}

parsing::ParseStrategyResult ReturnStatementNodeParseStrategy::parse(const tokenize::TokenStream &tokens) {
    ...
    auto [node, itr] = stream::workbench(tokens, [this, &tokens](tokenize::TokenStream &workbench) {
        ...
        auto return_node = std::make_shared<ReturnStatementNode>(tokens.current().value(), static_cast<std::shared_ptr<Expression>>(nullptr),
