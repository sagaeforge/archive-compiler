...
bool WhenExpressionNodeParseStrategy::can_parse(const parsing::Parser &parser, const tokenize::TokenStream &tokens) {
    return contains(tokens.current(), {tokenize::TokenType::When});
}

parsing::ParseStrategyResult WhenExpressionNodeParseStrategy::parse(parsing::Parser &parser, const tokenize::TokenStream &tokens) {
    ...
    auto [node, itr] = stream::workbench(tokens, [this, &parser](tokenize::TokenStream &workbench) {
        // current: 'when'
        auto whenToken = workbench.current();
        workbench.move_next();
        ...
        if (workbench.current().valid() && contains(workbench.current(), {tokenize::TokenType::LParen})) {
            ...
            auto [targetNode, targetItr] = expressionStrategy.parse(parser, workbench);
        ...
        do {
            ...
            if (workbench.current().valid() && contains(workbench.current(), {tokenize::TokenType::In})) {
                ...
                auto [expressionNode, expressionItr] = expressionStrategy.parse(parser, workbench);
            ...
            if (workbench.current().valid() && contains(workbench.current(), {tokenize::TokenType::LBrace})) {
                auto [blockStatementNode, blockStatementItr] = blockStatementStrategy.parse(parser, workbench);
        ...
        return std::make_shared<WhenExpressionNode>(whenToken.value(), target, conditions, alternative);
    });
